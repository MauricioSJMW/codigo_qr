  ...
  Future<String> verificarConexion(context, email, password) async {
    ...
    if (result == true) {
      ...
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => const SinConexion()));
      valor = 'dos';
    }
    return valor;
  }

  Future<String> verificarConexionBusqueda(
      ejercicio, reporte, tipo, usuario, context) async {
    bool result = await InternetConnectionChecker().hasConnection;
    var valor = '';
    if (result == true) {
      EscanerQrControlador buscarReporte = EscanerQrControlador();
      buscarReporte.busquedaManual(ejercicio, reporte, tipo, usuario, context);
      valor = 'uno';
    } else {
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => const SinConexion()));
